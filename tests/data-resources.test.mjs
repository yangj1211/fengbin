...
try {
  ...
  const { findSourceDocument, resolveSource } = require('./knowledge-sources.js');
  const {
    dataFiles,
    datasetReference,
    findDataFile,
  ...
  for (const agent of modules) {
    const own = resources.filter((item) => item.module === agent.id);
    assert.ok(
      own.some((item) => item.kind === '文件'),
      agent.id,
    );
    assert.ok(
      own.some((item) => item.kind === '数据表'),
      agent.id,
    );
  }
  for (const resource of resources) {
    assert.doesNotMatch(resource.name, /样例|示例|演示|虚构|合成/);
  ...
  for (const file of dataFiles) {
    const resource = resources.find(
      (item) => item.key === `file:${file.tableId}`,
    );
    const table = resources.find(
      (item) => item.key === `table:${file.tableId}`,
    ).table;
    assert.deepEqual(
      resource.table,
      table,
      'Preview and table must use the same records',
    );
    assert.equal(
  ...
  assert.equal(
    resources.find((item) => item.key === 'file:energy-details').table.rows
      .length,
    84,
  );
  assert.equal(
    resources.find((item) => item.key === 'file:production').table.rows.length,
    4,
  );
  assert.equal(
    resources.find((item) => item.key === 'file:suppliers').table.rows.length,
    3,
  );
  assert.equal(
    getDataResources(removeDataResource(state, 'file:production')).length,
    16,
  );
  assert.equal(
    getDataResources(state).length,
    17,
    'Deletion must not mutate the input state',
  );
  const reference = { documentId: 'catalog', sectionId: 'catalog-fb-lh470', page: 1 };
  const history = [{ question: '原始问题', sources: [reference] }];
  const deletedPdf = removeDataResource({ ...state, sessions: history }, 'file:catalog');
  ...
  assert.deepEqual(deletedPdf.sessions, history, 'Deletion retains historical answers and citation IDs');
  assert.ok(resolveSource(reference), 'Static reference validation must still preserve deleted references on hydration');
  const onlyTableDeleted = removeDataResource(state, 'table:production');
  assert.equal(isDataResourceDeleted(onlyTableDeleted, 'file:production'), false);
  assert.ok(getDataResources(onlyTableDeleted).some(item => item.key === 'file:production'));
  ...
  assert.equal(isDataResourceDeleted(removeDataResource(state, 'file:energy-details'), `file:${oldEnergy.tableId}`), true);
  const local = {
  ...
  assert.ok(
    !getDataResources({ datasets: [local] }).some(
      (item) => item.key === 'file:production',
    ),
  );
  const historical = {
  ...
  console.log(
    'Data resource tests passed: all five agents, matching file/table records, sources, deletion and legacy display.',
  );
} finally {
